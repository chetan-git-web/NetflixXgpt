...
const VideoTitle = ({ title, text }) => {
  return (
    <div className=' px-12 absolute bg-gradient-to-r from-black w-screen aspect-video pt-[20rem]' >
      <h1 className='text-6xl font-bold text-white'> {title}</h1>
      <p className='py-6 text-lg w-1/4 text-white'>{text}</p>
      <div className='flex space-x-2'>
        <button className='bg-white flex font-semibold p-6 rounded-md w-[6rem] h-[3rem] justify-center items-center hover:opacity-50'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24
